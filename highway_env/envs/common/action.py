...
class DiscreteAction(ContinuousAction):
    def __init__(self,
                 ...
                 dynamical: bool = False,
                 clip: bool = True,
                 actions_per_axis: Tuple[int, int] = (3, 3),
                 **kwargs) -> None:
    ...
    def space(self) -> spaces.Discrete:
        return spaces.Discrete(self.actions_per_axis[0]*self.actions_per_axis[1])

    def act(self, action: int) -> None:
        axis_1 = np.linspace(-1, 1, self.actions_per_axis[0])
        axis_2 = np.linspace(-1, 1, self.actions_per_axis[1])
        all_actions = list(itertools.product(axis_1, axis_2))
        #print('all actions available', len(all_actions))
        #print('action of choice', action)
