...
class KinematicObservation(ObservationType):
    ...
    FEATURES: List[str] = ['presence', 'x', 'y', 'vx', 'vy', 'lane_distance', 'lane_heading_difference']#,'destination_location_x','destination_location_y']
    ...
    def normalize_obs(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for feature, f_range in self.features_range.items():
            if feature in df:
                if feature == 'lane_distance' or feature=='heading':
                    continue
...
class OccupancyGridObservation(ObservationType):
    ...
    FEATURES: List[str] = ['presence', 'vx', 'vy', 'on_road', 'lane_distance', 'lane_heading_difference']
